...
import jsbgym
from jsbgym import aircraft, tasks, utils
from jsbgym.environment import JsbSimEnv, NoFGJsbSimEnv
from jsbgym.tests.stubs import BasicFlightTask
from jsbgym.visualiser import FlightGearVisualiser
    ...
    def test_init_spaces(self):
        ...
        for prop, lo, hi in zip(self.env.task.state_variables, obs_lows, obs_highs):
            self.assertAlmostEqual(
                lo,
                prop.min,
                msg=f"{prop} min of {prop.min} does not match space low of {lo}",
            )
            self.assertAlmostEqual(
                hi,
                prop.max,
                msg=f"{prop} max of {prop.max} does not match space high of {hi}",
            )
...
class TestGymRegistration(unittest.TestCase):
    def get_number_of_envs(self) -> int:
        ...
        return (
            num_tasks * num_flightgear_setttings * num_shaping_settings * num_aircraft
        )
